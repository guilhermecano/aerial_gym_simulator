...
class ModularAerialRobotCfg(BaseConfig):
    ...
    @classmethod
    def create(cls, asset_path):
        instance = cls()
        instance.asset_path = "/".join([f"{AERIAL_GYM_ROOT_DIR}",asset_path])
        drone_urdf = urdfpy.URDF.load(instance.asset_path)
        instance.num_actions = len([j for j in drone_urdf.joint_map.keys() if "propeller_joint" in j])
        ...
        instance.robot_asset = robot_asset()
